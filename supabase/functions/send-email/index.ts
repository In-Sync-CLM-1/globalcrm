...
const sendEmail = async (apiKey: string | undefined, to: string, subject: string, html: string, fromEmail: string, fromName: string, replyToEmail?: string, unsubscribeUrl?: string, inReplyTo?: string) => {
...
serve(async (req) => {
  ...
  try {
    ...
    const emailData = await sendEmail(resendApiKey, to, subject, finalHtml, fromEmail, fromName, replyToEmail, bareEmail ? undefined : unsubscribeUrl, inReplyTo);
