...
// Cron (every 5 min). Sends two attendee reminders per demo meeting:
...
async function sendEmail(supabase: any, orgId: string, to: string | null, subject: string, html: string): Promise<string> {
  ...
  const r = await fetch("https://api.resend.com/emails", {
    ...
    body: JSON.stringify({ from: `In-Sync <noreply@${es.sending_domain}>`, to: [to], subject, html }),
  });
...
Deno.serve(async () => {
  ...
  const now = new Date();
  const fromIso = new Date(now.getTime() - 10 * 60000).toISOString();
  const toIso = new Date(now.getTime() + 36 * 3600000).toISOString();

  const { data: meetings } = await supabase.from("contact_activities")
    .select("id, org_id, contact_id, subject, scheduled_at, meeting_link, demo_rsvp_status, demo_reminder_9am_sent_at, demo_reminder_1h_sent_at, demo_reminder_call_sent_at")
  ...
  for (const m of meetings || []) {
    ...
    if (!c) continue;
    if (c.opted_out) {
      const col0 = kind === "1h" ? "demo_reminder_1h_sent_at" : "demo_reminder_9am_sent_at";
      await supabase.from("contact_activities").update({ [col0]: new Date().toISOString() }).eq("id", m.id);
...
<p>See you there!<br>In-Sync</p></div>`;
    ...
    const col = kind === "1h" ? "demo_reminder_1h_sent_at" : "demo_reminder_9am_sent_at";
    await supabase.from("contact_activities").update({ [col]: new Date().toISOString() }).eq("id", m.id);
