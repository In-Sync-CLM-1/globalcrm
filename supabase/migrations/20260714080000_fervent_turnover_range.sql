-- Turnover min/max range filter needs a comparable number, not free text.
-- Adds a normalized-INR-millions column alongside the existing free-text
-- 'turnover' column (kept as-is for audit/fallback display when a value
-- can't be parsed, e.g. legacy USD-denominated rows).
ALTER TABLE public.fervent_data_repository
  ADD COLUMN IF NOT EXISTS turnover_inr_million numeric;

COMMENT ON COLUMN public.fervent_data_repository.turnover_inr_million IS
  'Turnover normalized to INR millions for the min/max range filter. NULL when the free-text turnover value could not be parsed (e.g. "$100B+" — no FX rate assumed).';

-- Backfill unambiguous INR crore/lakh legacy values. Anything containing
-- '$' is left NULL rather than guessing a USD->INR conversion rate.
UPDATE public.fervent_data_repository
SET turnover_inr_million = (regexp_match(turnover, '([0-9]+(?:\.[0-9]+)?)\s*(cr|crore)', 'i'))[1]::numeric * 10
WHERE turnover IS NOT NULL
  AND turnover NOT ILIKE '%$%'
  AND turnover ~* '[0-9](\.[0-9]+)?\s*(cr|crore)\y'
  AND turnover_inr_million IS NULL;

UPDATE public.fervent_data_repository
SET turnover_inr_million = (regexp_match(turnover, '([0-9]+(?:\.[0-9]+)?)\s*(lakh|lac)', 'i'))[1]::numeric * 0.1
WHERE turnover IS NOT NULL
  AND turnover NOT ILIKE '%$%'
  AND turnover ~* '[0-9](\.[0-9]+)?\s*(lakh|lac)\y'
  AND turnover_inr_million IS NULL;

-- Bonus data-hygiene fix: a stray raw employee count ("125") predates the
-- fixed-bucket filter and won't match any checkbox — bucket it the same way
-- normalizeEmployeeSize() would on a fresh edit.
UPDATE public.fervent_data_repository
SET employee_size = CASE
  WHEN employee_size::numeric <= 10 THEN '1-10'
  WHEN employee_size::numeric <= 50 THEN '11-50'
  WHEN employee_size::numeric <= 200 THEN '51-200'
  WHEN employee_size::numeric <= 500 THEN '201-500'
  WHEN employee_size::numeric <= 1000 THEN '501-1000'
  WHEN employee_size::numeric <= 5000 THEN '1001-5000'
  WHEN employee_size::numeric <= 10000 THEN '5001-10000'
  WHEN employee_size::numeric <= 100000 THEN '10001-100000'
  ELSE '100001+'
END
WHERE employee_size ~ '^[0-9]+$';

-- Carry turnover_inr_million through the CSV bulk-import upsert/merge paths
-- alongside the other fervent_data_repository columns.
...
BEGIN
  ...
  updated AS (
    ...
    SET
      ...
      turnover_inr_million = COALESCE(NULLIF(m.r->>'turnover_inr_million', '')::numeric, f.turnover_inr_million),
...
BEGIN
  ...
  WITH incoming AS (
    SELECT * FROM jsonb_to_recordset(p_records) AS r(
      ...
      sub_industry text, employee_size text, turnover text, turnover_inr_million numeric,
  ...
  upserted AS (
    INSERT INTO public.fervent_data_repository (
      ...
      turnover, turnover_inr_million, company_linkedin_url, created_by, import_job_id, upload_status
    )
    SELECT p_org_id, sr_no, unique_id, db_sourced_year, ucdb_status, company_name,
      ...
      direct_number, phone_number, official_email, personal_email_1, personal_email_2,
      linkedin_url, domain_name, website, industry, sub_industry, employee_size,
      turnover, turnover_inr_million, company_linkedin_url, p_created_by, p_import_job_id, 'fresh'
    FROM incoming
    ...
    DO UPDATE SET
      ...
      turnover_inr_million = EXCLUDED.turnover_inr_million,
