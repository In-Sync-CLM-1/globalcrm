-- email_volume in the Platform Command Center only ever counted
-- email_conversations (the manual inbox thread table). Automated pipeline
-- emails (IEDUP training notifications etc., sent via sendEmailTemplate in
-- _shared/pipelineActions.ts, ₹0.08/send) log to pipeline_email_log instead
-- and were never picked up — same class of gap as the Beneficiaries
-- Disposition column fix (email added as a channel 2026-07-28, downstream
-- views not extended). Count both sources.
